...
class CategoricalAttributeGenerator(AttributeGenerator):
    def __init__(self, name, values=10, domain=None, min_group=1, max_group=None, p=None):
    ...
    def create(self, variance=0):
        values = np.random.choice(self.values, np.random.randint(self.min_group, self.max_group + 1), replace=False)
        p = np.random.randint(1, variance + 2, len(values))
        p = p / np.sum(p)
        return CategoricalAttributeGenerator(name=self.name, values=values.tolist(), domain=self.domain, p=p)

    def random_value(self):
        return str(np.random.choice(self.values, p=self.p))
